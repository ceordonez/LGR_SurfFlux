...
def processing_data(data, cfg):
    ...
    results, p1, cdata = calculate_fluxes(cfg, data, pts_sel)

    return results, p1, cdata
...
def calculate_fluxes(cfg, data, pts):
    ...
    for pt in range(len(pts[0][::2])):
        ...
        points['Value start (ppm)'][pt] = data.iloc[idx1][cdata]
        points['Value end (ppm)'][pt] = data.iloc[idx2][cdata]
        ...
        pol,res, _, _, _ = np.polyfit(range(len(time[idx1:idx2+1])),
                                    data[cdata][idx1:idx2+1], 1, full=True)
        p = np.poly1d(pol)
        slope, intercept, r_value, p_value, std_err = \
            stats.linregress(range(len(time[idx1:idx2+1])),
                                data[cdata][idx1:idx2+1])
        p1.append(pol[1])
